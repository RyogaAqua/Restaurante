...
def update_menu_item(id_objeto, data):
    """
    Actualiza un elemento existente del menú en la base de datos.

    Args:
        id_objeto (int): El ID del elemento del menú a actualizar.
    ...
    """
    try:
        item = MenuObjetos.query.get(id_objeto)
        if not item:
        ...
        item.imagen_url = data.get('imagen_url', item.imagen_url)  # Actualizar la URL de la imagen
...
def delete_menu_item(id_objeto):
    """
    Elimina un elemento del menú de la base de datos.

    Args:
        id_objeto (int): El ID del elemento del menú a eliminar.
    ...
    try:
        ...
        db.session.commit()
        return {"message": f"Elemento del menú con ID {id_objeto} eliminado exitosamente."}
    except Exception as e:
        db.session.rollback()
