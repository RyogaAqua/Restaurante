...
CREATE TABLE IF NOT EXISTS `mydb`.`Orden` (
  `Id_Transaccion` INT NOT NULL AUTO_INCREMENT COMMENT 'Unique identifier for the order transaction',
...
CREATE TABLE IF NOT EXISTS `mydb`.`Orden_Items` (
  ...
  `Id_Transaccion` INT NOT NULL COMMENT 'Foreign key linking to the order header',
  ...
  INDEX `fk_Orden_Items_Orden_idx` (`Id_Transaccion` ASC) VISIBLE,
  INDEX `fk_Orden_Items_Menu_Objetos_idx` (`Id_Objeto` ASC) VISIBLE,
  UNIQUE INDEX `idx_unique_item_per_order` (`Id_Transaccion` ASC, `Id_Objeto` ASC) VISIBLE,
  CONSTRAINT `fk_Orden_Items_Orden`
    FOREIGN KEY (`Id_Transaccion`)
    REFERENCES `mydb`.`Orden` (`Id_Transaccion`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
